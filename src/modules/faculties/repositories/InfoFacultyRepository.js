...
class InfoFacultyRepository {
  ...
  async readAll() {
    return await db.InfoFaculty.findAll( {
      attributes: ['id', 'description', 'phoneNumber'],
      include: [
        {
          model: db.Faculty,
          as: 'faculty',
  ...
  async update(id, infoFaculty) {
    return await db.InfoFaculty.update(infoFaculty, { where: { id } });
  }
   ...
   */
  async destroy(id) {
    return await db.InfoFaculty.destroy({ where: { id } });
  }
...
module.exports = new InfoFacultyRepository();